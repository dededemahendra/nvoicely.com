...
type FolderColor = "brand" | "blue" | "black" | "grey" | "yellow" | "orange" | "red";
type FolderSize = "xs" | "sm" | "md" | "lg";
...
> = {
  ...
  xs: {
    container: "size-9 rounded-[11px]",
    tabLeft: "w-3.5 h-1.5 rounded-tl-sm",
    tabRight: "w-1 h-1.5 rounded-tr-[11px]",
    tabBridge: "w-1 h-1",
    flapBody: "h-3.5",
    papers: "inset-x-1.5 top-1",
    paperOffset: "top-0.5",
    paperH: "h-6",
...
> = {
  // Premium Ledger palette — forest folder, cream papers. Driven by theme
  // tokens so it adapts to both light and dark mode.
  brand: {
    folder: "from-primary to-primary/85",
    flap: "bg-primary-foreground/15",
    paperBack: "bg-primary-foreground/80",
    paperFront: "bg-primary-foreground",
    paperLine: "bg-primary/25",
    paperBorder: "border-primary/10",
    labelBg: "bg-primary-foreground/20",
    folderBorder: "border-white/15",
  },
  blue: {
...
export const Folder = ({
  ...
  return (
    <motion.div
      ...
      variants={{ rest: {}, hover: {} }}
      className={cn(
        "relative cursor-pointer border-t-2 bg-linear-to-b",
        s.container,
        c.folder,
        c.folderBorder,
        className
